...
export default function CommandPalette({ open, onClose, days, todayId, ctx }: CommandPaletteProps) {
  ...
  const actions: ActionItem[] = useMemo(() => {
    ...
    const items: ActionItem[] = [
      ...
      { kind: "action", id: "go-streak", label: "Go to Streak", hint: "quit-habit tracker", icon: <ShieldCheck className="w-4 h-4" />, keywords: "streak habit vicio abstinencia dias limpo sos", run: close(() => ctx.setView("streak")) },
      ...
      ...(FEATURES.shop
        ? [{ kind: "action" as const, id: "go-shop", label: "Go to Reward Shop", icon: <ShoppingBag className="w-4 h-4" />, keywords: "shop rewards points loja recompensas pontos", run: close(() => ctx.setView("shop")) }]
        : []),
